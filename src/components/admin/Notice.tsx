...
const AdminNotice: React.FC = () => {
  ...
  const fetchNotices = async () => {
    try {
      const response = await axios.get<Notice[]>(process.env.REACT_APP_API_URL + 'notice/allnotices');
      const activeNotices = response.data.filter((notice) => notice.status === 1);
      const sortedNotices = response.data.sort((a, b) => new Date(b.start_date).getTime() - new Date(a.start_date).getTime());
  ...
  const sendNoticeToStudent = async () => {
    try {
      ...
      const response = await axios.post(process.env.REACT_APP_API_URL + 'notice/createnotice', obj);
  ...
  const disableNotice = async (noticeId: number) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_API_URL}notice/removenotice/${noticeId}`);
      ...
      <Col md={6}>
        <div>
          ...
          <div className="mb-3">
            ...
            <input type="text" value={noticeTopic} onChange={(e) => setNoticeTopic(e.target.value)} />
          </div>
          <div className="mb-3">
            <label>Notice Content:</label>
            <textarea value={noticeContent} onChange={(e) => setNoticeContent(e.target.value)} rows={7} cols={50} style={{ verticalAlign: 'top' }} />
          </div>
          <div className="mb-3">
            <label>Due Date:</label>
            <input type="date" value={dueDate} onChange={(e) => setDueDate(e.target.value)} />
          </div>
          <Button variant="primary" onClick={sendNoticeToStudent}>Send Notice</Button>
          <br></br>
