...
const SubjectAdd: FC = () => {
    const subjectDateIntialState = { subjectName: "", subjectID: "", subjectAuthorName: "", subjectUploadDate: "", subjectUploadTime: "" }
    const navigate = useNavigate();
    const todayDate = new Date().toLocaleDateString('default', { year: 'numeric' }) + '-' + new Date().toLocaleString('default', { month: '2-digit' }) + '-' + new Date().toLocaleString('default', { day: '2-digit' });
    const todayTime = new Date().toLocaleString('dafault', { hour: '2-digit' }) + ":" + new Date().toLocaleString('default', { minute: '2-digit' }) + ":" + new Date().toLocaleString('default', { second: '2-digit' });
    ...
    return (
        <>
            <h4 className="bg-info bg-opacity-25 text-center py-2 mb-3">Create New Subject</h4>
            <Formik
