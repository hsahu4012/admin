...
const SubjectList: FC = () => {
    ...
    const deleteSubject = async (id: any) => {
        ...
        if (val == true) {
            const url = process.env.REACT_APP_API_URL + 'subjects/removesubject/' + id;
    ...
    return (
        <>
            <br></br>
            <Link to="/subjectadd" className='btn btn-primary'>Create New Subject</Link>
            <br></br>
                ...
                <tbody>
                    {
                        subjectList && subjectList.map((item, index) => (
                            <tr key={index + item.subject_id}>
                                ...
                                <td>
                                    <Link to={`/subjectedit/${item.subject_id}`} className='btn btn-warning'>Edit</Link>
                                </td>
                                <td>
                                    <button onClick={() => deleteSubject(item.subject_id)} className='btn btn-primary'>Delete Subject</button>
                                </td>
                            </tr>
                        ))
                    }
