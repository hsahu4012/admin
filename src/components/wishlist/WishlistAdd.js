...
const WishlistAdd = () => {
    ...
    const submitWishlist = async (values) => {
      ...
      if (confirmed) {
        await axios.post(
          `${process.env.REACT_APP_API_URL}wishlist/addToWishlist`,values
        );
        navigate("/wishlist");
      }
    } catch (error) {
  ...
  return (
    <>
   <Formik enableReinitialize={true} initialValues={FormValues} onSubmit={(values)=>submitWishlist(values)}>
   <Form>
